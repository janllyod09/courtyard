            ...
            <div class="mb-6 flex flex-col sm:flex-row items-end justify-between">
                <div class="w-full sm:w-1/3 sm:mr-4">
                    ...
                    <input type="text" id="search" wire:model.live="search" class="px-2 py-1.5 block w-full shadow-sm sm:text-sm border border-gray-400 hover:bg-gray-300 rounded-md
                            ...
                            dark:text-gray-300 dark:bg-gray-800" placeholder="Enter company or registered name">
                </div>

                <div class="w-full sm:w-2/3 flex flex-col sm:flex-row sm:justify-end sm:space-x-4">
                    <!-- Export to Excel -->
                    <div class="relative inline-block text-left">
                        <button wire:click="exportClients" class="peer mt-4 sm:mt-1 inline-flex items-center dark:hover:bg-slate-600 dark:border-slate-600
                            ...
                            <img class="flex dark:hidden" src="/images/export-excel.png" width="22" alt=""
                                wire:loading.remove wire:target="exportClients">
                                                    ...
                                                    <tr class="text-neutral-800 dark:text-neutral-200">
                                                        <td class="px-2 py-2 flex justify-center items-center">
                                                            <div class="flex justify-center items-center cursor-pointer"
                                                                ...
                                                                @else
                                                                <img class="w-full h-full rounded-full hover:grayscale"
                                                                    src="{{ asset('images/blank-profile.png') }}" alt=""
                                                                    style="pointer-events:none">
                                                                @endif
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td
                                                            class="px-5 py-4 text-center text-sm font-medium whitespace-nowrap uppercase">
                                                            {{ $client->mining_type }}
                                                        </td>
                                                        <td
                                                            class="px-5 py-4 text-center text-sm font-medium whitespace-nowrap">
                                                        ...
                                                        <td
                                                            ...
                                                            <div class="relative">
                                                                {{-- <button
                                                                    wire:click="toggleEditRole({{ $client->id }})"
                                                                ...
                                                                <button
                                                                    wire:click="toggleDelete({{ $client->id }}, 'role')"
        ...
        <div class="p-4">
            <div class="flex flex-col sm:flex-row gap-4 sm:items-center">
                ...
                <img src="{{ route('profile-photo.file', ['filename' => basename($selectedClient->profile_photo_path)]) }}"
                    alt="{{ $selectedClient->name }}" width="100" height="100" class="w-20 h-20 rounded-full">
                @else
                <img class="w-20 h-20 rounded-full" src="{{ $selectedClient->profile_photo_url }}" width="100"
                    height="100" alt="" />
                @endif
                <div>
                    <h1 class="text-2xl font-bold text-gray-700 dark:text-gray-100">{{ $selectedClient->company_name }}
                    </h1>
            ...
            <div class="text-sm">
                <div>
                    <p class="ml-0 sm:ml-4">Permit/Contract No.: <span class="font-bold">{{
                            $selectedClient->permit_number }}</span></p>
                </div>
                ...
                <div>
                    <p class="ml-0 sm:ml-4">Product: <span class="font-bold">{{ implode(', ',
                            safeJsonDecode($selectedClient->product)) }}</span></p>
                </div>

                <div>
                    <p class="ml-0 sm:ml-4">Permit Type: <span class="font-bold uppercase">{{
                            $selectedClient->permit_type }}</span></p>
                </div>

                <div>
                    <p class="ml-0 sm:ml-4">Permit Location: <span class="font-bold">{{ $selectedClient->location
                            }}</span></p>
                </div>
            </div>
